...
import (
	...
	cluster "github.com/hashicorp/terraform-provider-azurerm/internal/services/redhatopenshift"
	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			...
			Check: acceptance.ComposeTestCheckFunc(
				...
				check.That(data.ResourceName).Key("master_profile.0.vm_size").HasValue(string(cluster.StandardD8sV3)),
				check.That(data.ResourceName).Key("worker_profile.0.vm_size").HasValue(string(cluster.StandardD4sV3)),
				...
				check.That(data.ResourceName).Key("api_server_profile.0.visibility").HasValue(string(cluster.APIPublic)),
